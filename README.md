...
В этой лабораторной работе я изучила создание пользовательских типов данных и полиморфизм в Clojure. Получилось отделить интерфейс словаря от его реализации через протоколы - протокол `IDict` описывает, что должен уметь словарь, а `deftype RBDict` реализует это через красно-черное дерево. В будущем можно добавить другие реализации (через AVL-дерево или хеш-таблицу), не меняя интерфейс.
